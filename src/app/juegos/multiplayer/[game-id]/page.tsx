...
export default function TicTacToe() {
  ...
  // 1. Obtener usuario actual del sessionStorage
  useEffect(() => {
    const name = sessionStorage.getItem('name');
    setCurrentUser(name);
  }, []);

  // 2. Cargar estado inicial del juego
  useEffect(() => {
          ...
          <div className="grid grid-cols-3 gap-2 mb-6">
            {gameState.board.map((cell, index) => (
              <div
                ...
                className={`h-24 flex items-center justify-center text-4xl font-bold rounded-lg transition-colors
              ${cell === 1 ? 'bg-blue-100 text-blue-600' :
                  cell === 2 ? 'bg-red-100 text-red-600' :
                  isCurrentTurn ? 'bg-black-200 hover:bg-gray-300 cursor-pointer' : 'bg-black'}
              ${isCurrentTurn && cell === 0 ? 'ring-2 ring-gray-300' : ''}`}
              >
                {cell === 1 ? 'X' : cell === 2 ? 'O' : ''}
              </div>
            ))}
        ...
        <div className="col-span-1">
          <div className="mb-4 text-center">
            {gameState.winner === 'draw' ? (
              <p className="text-yellow-600 font-bold">¡Empate!</p>
            ) : gameState.winner ? (
